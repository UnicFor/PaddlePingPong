...
import { processSmoothingData } from '@/utils/dataFitting.js';
...
const props = defineProps({
  ...
  smoothMethod: {
    type: String,
    default: 'movingAverage' // 'movingAverage' | 'exponential'
  },
  smoothParam: {
    type: Number,
    default: 2 // 窗口大小或alpha值
  }
});
...
const processedData = computed(() => 
  processSmoothingData(props.data, props.smoothMethod, props.smoothParam)
);
  ...
  const option = {
    ...
    series: [
      ...
      },
      {
        name: '速度 (平滑)',
        type: 'line',
        data: processedData.value.speedSmoothed,
        symbol: 'none',
        itemStyle: {
          color: '#ff7875',
        },
...
watch(() => [props.smoothMethod, props.smoothParam], () => {
  initChart();
});
</script>
